...
This slice has started in the existing [UI framework spike](../poc/findings/ui-framework-spike.md) with mocked `localStorage` persistence, offline mode, mutation queue states, sync, supervisor review, approval, static Odoo/OpenProject-shaped fixture modes, and a first [Adapter Contract](adapter-contract.md). The [offline PWA test plan](../poc/offline-pwa-test-plan.md) remains the reliability test once a real local database and sync engine are selected.
